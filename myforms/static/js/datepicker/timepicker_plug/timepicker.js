...
jQuery.fn.datetime = function() {
	...
	return this.each(function(){
				...
				function writeDelegate(fragment){
					writeDate(fragment);
					closePickPlug({target: ''});
				}
				var datepicker_def = {
							changeMonth: true,
							changeYear: true,
					...
					if (!$('#pickerplug').length) {
						...
						$('#hourSlider').slider({
							...
							change: function(event, ui) {
								var node = $('#tpSelectedTime .selMins')
								if(node.effect && typeof(node.effect) === 'function'){
									node.effect('highlight', 1000);
								}
							}
						});
						// Slider
...
//								hours = '0'.concat(hours);
								hours = jQuery.sprintf("%02d", hours);
							}
							if (fragment < 10) {
//								fragment = '0'.concat(parseInt(fragment));
								fragment = jQuery.sprintf("%02d", fragment);
							}
							...
							break;
						case 'minute':
							minutes = jQuery.sprintf("%02d", fragment);
							$('#tpSelectedTime .selMins').text(minutes);
							hour = $('#hourSlider').slider('option', 'value');
							time = jQuery.sprintf("%02d", hour)+':'+minutes;
							break;
					}
					return time;
				}
				...
				function closePickPlug (event) {
					var pickerNode = $('#pickerplug');
					if (!pickerNode.is(":visible")) {
						return;
					}
